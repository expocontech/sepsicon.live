...
class FacultyDesign extends React.Component {
  ...
  toggleModal = (fid) => {
    ...
    if (sessionStorage.getItem("token") != null) {
      ...
      axios
        .get(`${process.env.REACT_APP_BASENAME}faculty/${fid}`, {
          headers: authHeader,
        })
        .then((response) => {
          this.setState({
  ...
  componentDidMount = () => {
    if (sessionStorage.getItem("token") != null) {
      ...
      axios
        .get(`${process.env.REACT_APP_BASENAME}faculty/factype`, {
          headers: authHeader,
        })
        .then((response) => {
      ...
      axios
        .get(`${process.env.REACT_APP_BASENAME}faculty`, {
          headers: authHeader,
        })
        .then((response) => {
          this.setState({
  ...
  renderCards = () => {
    let result = this.state.factype.map((itemtype) => {
      return (
        <Col sm="12" style={{ paddingLeft: "10px", paddingRight: "10px" }}>
          <h1
            style={{
              color: "#5385c5",
              marginBottom: "20px",
                  ...
                  return (
                    ...
                    >
                      ...
                      <Card style={{ background: "none" }}>
                        <CardBody className="speaker">
                          <img
                            src={
                              `${process.env.REACT_APP_BASENAME}` + item.photo
                            }
                            alt={item.name}
  ...
  render() {
    return (
      <Row>
        ...
        >
          <ModalHeader toggle={this.toggleModal}>Faculty Details</ModalHeader>
          <ModalBody>
            <Form>
              <Row>
                <Col md="12" sm="12" xs="12">
                  <img
                    src={
                      `${process.env.REACT_APP_BASENAME}` + this.state.dphoto
                    }
                    className="mr-2"
                    width="170"
                  ...
                  >
                    Designation : {this.state.ddesignation}
                    <br />
                    Institute : {this.state.dinstitute}
                  </p>
